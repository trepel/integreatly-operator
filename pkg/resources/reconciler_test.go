...
func TestNewReconciler_ReconcileSubscription(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			FakeMPM: &marketplace.MarketplaceInterfaceMock{
				CreateSubscriptionFunc: func(ctx context.Context, os v13.OperatorSource, ns string, pkg string, channel string, operatorGroupNamespaces []string, approvalStrategy alpha1.Approval) error {
					return nil
				},
				GetSubscriptionInstallPlanFunc: func(ctx context.Context, subName string, ns string) (plan *alpha1.InstallPlan, subscription *alpha1.Subscription, e error) {
	...
	for _, tc := range cases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			status, err := reconciler.ReconcileSubscription(context.TODO(), tc.SubscriptionName, "test-ns", tc.client)
